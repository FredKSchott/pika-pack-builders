...
function getPikaGlobalDir(): string | null {
  const pikaPrefix = getPikaGlobalPrefix();
  if (!pikaPrefix) {
    return pikaPrefix;
  }
  return path.join(
    path.resolve(pikaPrefix),
...
export async function build({cwd, out, reporter, isFull, manifest}: BuilderOptions): Promise<void> {
  ...
  const tsc =
    importFrom.silent(cwd, 'typescript') ||
    (globalPackageDir && importFrom.silent(globalPackageDir, 'typescript')) ||
    importGlobal.silent('typescript');
