...
export async function build({ out, cwd, options, reporter }) {
  const relativeOutWasm = path.relative(
    cwd,
    path.join(out, "assets/index.wasm")
  );
  const relativeOutTypes = path.relative(
    cwd,
    path.join(out, "assets/index.d.ts")
  );
